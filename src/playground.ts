...
let INPUTS: {[name: string]: InputFeature} = {
  "x": {f: (x, y) => x, label: "Blüte"},
  "y": {f: (x, y) => y, label: "Blatt"},
  "xSquared": {f: (x, y) => x * x, label: "Blüte^2"},
  "ySquared": {f: (x, y) => y * y,  label: "Blatt^2"},
  "xTimesY": {f: (x, y) => x * y, label: "Blüte*Blatt"},
  "sinX": {f: (x, y) => Math.sin(x), label: "sin(Blüte)"},
  "sinY": {f: (x, y) => Math.sin(y), label: "sin(Blatt)"},
};
...
function updateHoverCard(type: HoverType, nodeOrLink?: nn.Node | nn.Link,
  ...
  let name = (type === HoverType.WEIGHT) ? "Gewicht" : "Präferenz";
  hovercard.style({
